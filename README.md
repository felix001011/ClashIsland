...
- **跟随 Clash 客户端**：Clash 关闭超过 8 秒悬浮岛自动隐身，重新打开 Clash 自动现身；配合看门狗任务，即使悬浮岛被退出或意外结束，Clash 在跑时也会在 1 分钟内自动回归
...
- 启用：双击 `autostart-on.vbs`——同时注册两样东西：登录时自动启动（启动文件夹快捷方式）+ 看门狗计划任务（每分钟检查一次，Clash 在跑而悬浮岛不在就自动拉起）
- 取消：双击 `autostart-off.vbs`——两样一起移除

右键菜单的"退出悬浮岛"只对**本次 Clash 会话**生效：退出后看门狗不会烦你，但下次重开 Clash 悬浮岛会自动回归。想彻底停用就用 `autostart-off.vbs`。
...
| `start.vbs` | 无窗口启动器 |
| `autostart-on.vbs` / `autostart-off.vbs` | 开机自启 + 看门狗开关 |
| `watchdog.vbs` | 看门狗（由计划任务每分钟调用，无需手动运行） |
...
Clash 关闭超过 8 秒后悬浮岛会自动隐身，重新打开 Clash 会自动现身。

**如何完全退出？**
右键悬浮岛 → 退出悬浮岛（本次 Clash 会话内不再出现，Clash 重启后回归）。
要永久停用自动启动，双击 `autostart-off.vbs`。
